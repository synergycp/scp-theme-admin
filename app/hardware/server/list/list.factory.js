(function () {
  ...
  angular.module("app.hardware").factory("ServerList", ServerListFactory);
  ...
  function ServerListFactory(
    ...
    return function () {
      ...
      */
      list.bulk.add("Assign Billing Date", handler(ServerAssign.billing.date));
      ...
      list.bulk.add($translate.instant("server.modal.launch-kvm.TITLE"), handler(ServerAssign.launchKvm));
      return list;

      function switchCommands(items) {
        return Modal.make({
          templateUrl:
            "app/hardware/server/switch/switch.modal.bulk-commands.html",
