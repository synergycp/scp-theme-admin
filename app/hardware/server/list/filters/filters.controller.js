(function () {
  ...
  function ServerFiltersCtrl(
    ...
    filters.disks = MultiInput(DiskSelector).add().setMax(50);
    filters.addons = MultiInput(AddonSelector).add().setMax(8);
    filters.bw = {
      min: null,
      max: null,
    };
    ...
    function init() {
      ...
      var promises = [
        ...
        filters.client.setSelectedId(
          $state.params["client"] || filters.current.client
        ),
        ...
        filters.addons.setSelectedId(
          ($state.params["addons[]"] || []).join(",")
        ),
        filters.billing.integration.setSelectedId(
          $state.params["billing.integration"]
        ),
      ];
    ...
    function fireChangeEvent() {
      _.assign(filters.current, {
        ...
        "billing.id":
          (filters.billing.integration.selected && filters.billing.id) ||
          undefined,
        "billing.integration":
          filters.billing.integration.selected &&
          filters.billing.integration.selected.id,
    ...
    function DiskSelector(selected) {
      ...
      select.setSelectedId(
        (selected && (_.isString(selected) ? selected : selected.id)) || null
      );
