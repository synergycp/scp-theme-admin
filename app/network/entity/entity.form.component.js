(function () {
   ...
   * @ngInject
   */
  function EntityFormCtrl(_, Api, Select) {
    var entityForm = this;
    ...
    entityForm.groups = Select("group");
    ...
    function getV4RangeEnd() {
      ...
      var countDots = (entityForm.v4.range_end.match(RegExp("\\.", "g")) || [])
        .length;
    ...
    function incrementBulk() {
      ...
      entityForm.input.v4.address = ipHandler(
        entityForm.input.v4.address,
        ip_increment
      );

      if (entityForm.v4.range_end) {
        entityForm.v4.range_end = (
          parseInt(entityForm.v4.range_end) + parseInt(ip_increment)
        ).toString();
      }

      if (increment_vlan) {
        entityForm.input.vlan++;
        entityForm.input.v4.gateway = ipHandler(
          entityForm.input.v4.gateway,
          ip_increment
        );
      }
      entityForm.input.nickname = nicknameHandler(
        entityForm.input.nickname,
        ip_increment
      );
    }
  }
